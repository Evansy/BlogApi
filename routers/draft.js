...
router.post('/api/draft', confirmToken, (req, res) => {
    ...
    res.status(200).send(JSON.stringify({message: 'succeed in saving new draft'}));
})
...
router.patch('/api/draft/:aid', confirmToken, (req, res) => {
    ...
    db.Article.update({aid: aid}, article, (err, data) => {
        ...
        if(err){
            ...
            res.status(200).send(JSON.stringify({message: 'succeed in updating ---' + data.title}))
        }
    })
})
...
router.get('/api/drafts', (req, res) => {
    ...
    db.Article.find({isPublish: false}).sort({date: -1}).limit(limit).skip(skip).exec()
        .then(articles => {
